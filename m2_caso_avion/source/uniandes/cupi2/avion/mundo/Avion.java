...
public class Avion {
    ...
    /**
     * Asigna la silla a un pasajero, seg�n sus preferencias de clase y ubicaci�n
     * <br>
     ...
     * @param ubicacion - ubicaci�n elegida por el pasajero - si clase =
     *                  CLASE_ECONOMICA entonces ubicaci�n pertenece {VENTANA,
     *                  CENTRAL, PASILLO}, o si clase = CLASE_EJECUTIVA
     *                  entonces ubicaci�n pertenece {VENTANA, PASILLO}
     * @param pasajero  - pasajero a asignar - pasajero != null y no tiene silla en
     *                  el avi�n
     ...
     * indicada.
     * 
     * @param ubicacion - ubicaci�n en donde buscar la silla - ubicaci�n pertenece
     *                  {VENTANA, PASILLO}
     ...
     * @return true si encontr� el pasajero y desasign� la silla, false en caso
     *         contrario
     */
    ...
    public int darDuracionTotalPeliculas() {
        Silla[] economicsSeats = obtenerSillasEconomicas();
        Silla[] executiveSeats = obtenerSillasEjecutivas();
        ArrayList<Pelicula> movies = new ArrayList<Pelicula>();

        int movieDuration = 0;
        for (int i = 0; i < economicsSeats.length; i++) {
            if (economicsSeats[i].sillaAsignada()) {
                movies = economicsSeats[i].darPasajero().darPeliculas();
                for (int j = 0; j < movies.size(); j++) {
                    movieDuration += movies.get(j).darDuracion();
                }
            }
        }

        for (int i = 0; i < executiveSeats.length; i++) {
            if (executiveSeats[i].sillaAsignada()) {
                movies = executiveSeats[i].darPasajero().darPeliculas();
                for (int j = 0; j < movies.size(); j++) {
                    movieDuration += movies.get(j).darDuracion();
                }
            }
